...
export async function repartitionParProjet(moi, sprintId = null) {
  const entrees = await prisma.entree.findMany({
    ...
    select: {
      ...
      projetRef: {
        select: {
          id: true, libelle: true, ticket: true,
          porteurs: { select: { developpeur: { select: { id: true, nom: true } } } },
        },
      },
    },
  });
  ...
  for (const e of entrees) {
    ...
    const acc = parProjet.get(cle) ?? {
      libelle,
      ticket: e.projetRef?.ticket ?? null,
      // Porteurs déclarés sur le projet, pas seulement ceux qui ont saisi.
      porteurs: e.projetRef?.porteurs.map((x) => x.developpeur.nom) ?? [],
      engage: 0, reel: 0, sujets: 0,
    };
