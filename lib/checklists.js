...
/**
 * Statuts qui ouvrent les instances de validation projet (DAB, CAB ACL, Go Live).
 * On les énumère au lieu de comparer `ordre` : les états d'exception INCIDENT et
 * BLOQUE portent un ordre supérieur (11 et 12) sans être plus avancés dans le
 * processus. Un seuil « ordre >= PASSAGE_DAB » retenait donc un ticket bloqué en
 * implémentation et laissait de côté un ticket dont le DAB est validé.
 */
export const STATUTS_VALIDATION = ['PASSAGE_DAB', 'CAB_ACL', 'CAB_GO_LIVE', 'LIVE'];

/** Un ticket relève du suivi des checklists s'il est au stade de validation, ou si une checklist a déjà été ouverte pour lui. */
export const suitDesChecklists = (entree) =>
  STATUTS_VALIDATION.includes(entree.execution) || (entree.checklists?.length ?? 0) > 0;
 ...
 */
export async function checklistManquantePour(prisma, execution, { sprintId, entreeId, forcer = false }) {
  if (!forcer && entreeId) {
    const entree = await prisma.entree.findUnique({
      where: { id: entreeId },
      select: { projetRef: { select: { suiviChecklist: true } } },
    });
    if (entree?.projetRef?.suiviChecklist === false) return null;
  }
