...
/**
 * Rôles qui produisent de la capacité de développement. Le Scrum Master anime le
 * sprint sans porter de charge : l'y inclure gonflerait la capacité de la squad
 * d'une personne qui ne code pas.
 */
export const ROLES_CAPACITE = ['TECH_LEAD', 'DEVELOPPEUR'];
...
const DROITS = {
  SUPER_ADMIN: [
    ...
    'checklist.cocher', 'checklist.valider', 'checklist.gerer',
  ],
  SCRUM_MASTER: [
    ...
    'checklist.cocher', 'checklist.valider',
  ],
  TECH_LEAD: [
    'dashboard.voir', 'export.csv',
    'entree.creer.soi', 'entree.modifier.soi', 'entree.supprimer.soi',
  ...
  OBSERVATEUR: [
    // Lecture seule bornée à sa propre squad : « dashboard.tout » (vision
    // transverse) reste l'apanage du super admin.
    'dashboard.voir', 'export.csv',
  ],
};
...
/**
 * Rôles dont l'utilisateur peut consulter les droits : les siens, plus ceux
 * qu'il administre. Un Scrum Master n'a pas à voir la colonne « Super admin »,
 * pas plus qu'il n'en a l'écran.
 */
export function rolesVisibles(moi) {
  if (peut(moi, 'compte.gerer')) return Object.keys(ROLES);
  const siens = moi?.role && ROLES[moi.role] ? [moi.role] : [];
  if (!peut(moi, 'compte.gerer.squad')) return siens;
  return [...new Set([...siens, ...ROLES_DELEGABLES])];
}
...
export const MATRICE = [
  ...
  ['Cocher les items d’une checklist', 'checklist.cocher'],
  ['Valider une checklist (SDD, DAB, CAB…)', 'checklist.valider'],
  ['Gérer le référentiel des checklists', 'checklist.gerer'],
