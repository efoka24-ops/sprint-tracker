...
/** Un projet engagé doit être actif ET explicitement planifié sur un sprint. */
export const compteDansEngagement = (projet) => STATUTS_PROJET[projet?.statut]?.engage === true && !!projet?.sprintId;
...
export function engagement(projets = []) {
  ...
  return {
    ...
    exclus: exclus.map((p) => ({
      ...
      libelle: p.libelle,
      statut: p.statut,
      heures: p.heuresFaisabilite || 0,
      raison: p.sprintId ? (STATUTS_PROJET[p.statut]?.label ?? p.statut) : 'Non planifié',
    })),
