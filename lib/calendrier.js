 ...
 */
export function capaciteSemaine({ dateDebut, dateFin, membres, conges, feries, heuresParJour = 8 }) {
  ...
  const detail = membres.map((m) => {
    ...
    return { membre: m, joursDisponibles: jours, joursAbsents: absences, heures: jours * heuresParJour };
  });
