...
export async function calculerBilan(sprintId) {
  try {
    ...
    // Un objectif est tenu uniquement s'il est livré en LIVE.
    const realises = entrees.filter((e) => e.execution === 'LIVE').length;
    const valides = entrees.filter((e) => e.valide).length;
    const enTestQualif = entrees.filter((e) => ['TEST_QUALIF', 'RETOUR_QUALIF', 'TEST_BUSINESS', 'RETOUR_BUSINESS', 'PASSAGE_DAB', 'CAB_ACL', 'CAB_GO_LIVE', 'LIVE'].includes(e.execution)).length;
    const enTestBusiness = entrees.filter((e) => ['TEST_BUSINESS', 'RETOUR_BUSINESS', 'PASSAGE_DAB', 'CAB_ACL', 'CAB_GO_LIVE', 'LIVE'].includes(e.execution)).length;
    ...
    const livres = entrees.filter((e) => e.execution === 'LIVE').length;
    ...
    return {
      stats: {
        total, realises, valides, enCours, bloques, livres,
        tauxRealisation: pourcent(realises, total),
        tauxValidation: pourcent(valides, total),
        tauxTestQualif: pourcent(enTestQualif, total),
        tauxTestBusiness: pourcent(enTestBusiness, total),
...
export async function constatsAutomatiques(sprintId) {
  ...
  const valides = entrees.filter((e) => e.execution === 'LIVE');
